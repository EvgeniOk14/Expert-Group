...
public class LogFileWebSocketHandler extends TextWebSocketHandler
{
    ...
    public LogFileWebSocketHandler(LogFileService logFileService)
    {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws IOException {
        String payload = message.getPayload(); // получение сообщения от клиента
        System.out.println("Сообщение от клиента: " + payload);

        if (payload.startsWith("get log "))
        {
            String partialFileName = payload.substring(8);
            System.out.println("Часть имени файла, т.е. номер платы: " + partialFileName);

            //String logContent = logFileService.getLogFileContentByName(partialFileName); // получение по номеру платы содержимого файла
              String logContent = logFileService.getLogFileContent(partialFileName); // получение по номеру платы содержимого файла
            if (logContent != null)
            {
