...
public class ParseServerLazy
{
    ...
    public void onMessage(Session session, String message) throws IOException
    {
        if (message != null && !message.trim().isEmpty()) // если сообщение не пустое и не равно null
        {
            ...
            {
                System.out.println("Производим инициализацию parseMessage!!!");
                ...
                parseMessage = new ParseMessage(handleGetAction, handleCreateAction,
            ...
            else
            {
                ...
                System.out.println(" берём parseMessage с предидущей инициализации!!!");
                //parseMessage.parseMessage(message, session); // метод парсит строку и производит действия в зависимости от результата

            }
            parseMessage.parseMessage(message, session); // метод парсит строку и производит д
        }
        else
        {
    ...
    public static void main(String[] args)
    {
                ...
                // переопределяет точки для настройки, такие как метод configure
        {
            ...
            {
                factory.register(ParseServerLazy.class); // регистрация класса EchoServer
            }
        };
...
//                                                     handlePostAction, handleSaveAction,
//                                                     handleSendAction);
//        ParseServerLazy.setParseMessage(parseMessage); // передача parseMessage в ParseServer
