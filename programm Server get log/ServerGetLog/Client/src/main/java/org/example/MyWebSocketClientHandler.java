...
public class MyWebSocketClientHandler
{
    ...
    {
        System.out.println("Connected to server: " + session.getRemoteAddress().getAddress()); // печать в консоль адреса сервера
        try
        {
            session.getRemote().sendString("get log 12345ABC"); // Отправка сообщения серверу на получение содержимого из файлов
            session.getRemote().sendString("get log 67890DEF"); // Отправка сообщения серверу на получение содержимого из файлов
           // session.getRemote().sendString("get log test");
        }
        catch (Exception e)
        {
            e.printStackTrace();
